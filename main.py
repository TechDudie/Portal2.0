...
while 1:
  try:
    ...
    for line in temp:
        ...
        if msg == "$random":
            send("PRIVMSG {} :{}\r\n".format(CHANNEL, random.randint(
                0, 100)).encode("utf-8"))
        if msg == "$die":
            send("PRIVMSG {} :*death by frying pan*\r\n".format(CHANNEL).encode("utf-8"))
        if msg.startswith("$die "):
            send("PRIVMSG {} :*{} is dead by frying pan*\r\n".format(CHANNEL, tokens[1]).encode("utf-8"))
        if msg == "$kill":
            send("PRIVMSG {} :*death by nuclear anihlation*\r\n".format(CHANNEL).encode("utf-8"))
        if msg.startswith("$kill "):
            send("PRIVMSG {} :*{} is dead by nuclear anihlation*\r\n".format(CHANNEL, tokens[1]).encode("utf-8"))
    ...
    send("PRIVMSG {} :Sorry, what? You caused an error.\r\n".format(CHANNEL).encode("utf-8"))